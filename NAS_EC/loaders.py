...
class Rescale(object):
    ...
    def __call__(self, sample):
        ...
        elif isinstance(target, np.ndarray) and target.shape[-2] > 1 and target.shape[-1] > 1:
...
class ColorConvert(object):
    ...
    def __call__(self, sample):
        ...
        if self.target_weight is not None and isinstance(target, np.ndarray) and target.shape[-2] > 1 and target.shape[-1] > 1:
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        if target is None:
            return torch.from_numpy(image.copy()), None

        if isinstance(target, np.ndarray) and target.shape[-2] > 1 and target.shape[-1] > 1:
...
class ImageDataset(Dataset):
    def __init__(self, root_dir, available_indices=None, transform=None):
...
class Image2ImageDataset(ImageDataset):
    def __init__(self, root_dir, task_folder, available_indices=None, transform=None):
        super(Image2ImageDataset, self).__init__(root_dir, available_indices, transform)
...
class Image2LabelDataset(ImageDataset):
    def __init__(self, root_dir, task_folder, available_indices=None, transform=None, n_labels=1):
        super(Image2LabelDataset, self).__init__(root_dir, available_indices, transform)
        self.task_folder = task_folder
    ...
    def _getTarget(selfm, idx):
        return self.gt_labels[idx]


class Image2PositionsDataset(Image2LabelDataset):
    def __init__(self, root_dir, task_folder, subtask_folder, available_indices=None, transform=None):
        super(Image2PositionsDataset, self).__init__(root_dir, task_folder, subtask_folder, available_indices, transform)
...
class StareDiagnoses(Image2LabelDataset):
    def __init__(self, root_dir, available_indices=None, transform=None):
        super(StareDiagnoses, self).__init__(root_dir, 'Classification', 'Goldstandards', available_indices, transform, n_labels=15)

    def _loadGrountruth(self, available_indices=None):
        self.gt_labels = np.empty([len(self.im_file_list), self.n_labels], dtype=np.int64)
        fp = open(os.path.join(root_dir, 'Classification', 'all-mg-codes.csv'), 'r')
...
class StareOpticNerve(Image2PositionsDataset):
    def __init__(self, root_dir, available_indices=None, transform=None):
        super(StareOpticNerve, self).__init__(root_dir, 'OpticNerve', available_indices, transform)
...
class DriveSegmentation(Image2ImageDataset):
    def __init__(self, root_dir, transform=None, train=False):
        ...
        super(DriveSegmentation, self).__init__(root_dir, 'Segmentation', available_indices, transform)
...
class DriveFOV(Image2ImageDataset):
    def __init__(self, root_dir, transform=None, train=False):
        ...
        super(DriveFOV, self).__init__(root_dir, 'FOV', available_indices, transform)


class CHASEDB1Segmentation(Image2ImageDataset):
    def __init__(self, root_dir, available_indices=None, transform=None):
        super(CHASEDB1Segmentation, self).__init__(root_dir, 'Segmentation', available_indices, transform)
...
class HRFSegmentation(Image2ImageDataset):
    def __init__(self, root_dir, available_indices=None, transform=None):
        super(HRFSegmentation, self).__init__(root_dir, 'Segmentation', available_indices, transform)
        

class HRFFOV(Image2ImageDataset):
    def __init__(self, root_dir, available_indices=None, transform=None):
        super(HRFFOV, self).__init__(root_dir, 'FOV', available_indices, transform)
        

class HRFOpticNerve(Image2PositionsDataset):
    def __init__(self, root_dir, available_indices=None, transform=None):
        super(HRFOpticNerve, self).__init__(root_dir, 'OpticNerve', available_indices, transform)
...
if __name__ == '__main__':
    ...
    composed = transforms.Compose([Rescale((565, 565)), RandomFlip(), RandomRotation(center_offset=False), ToTensor()])
    hrf_ds = HRFOpticNerve(root_dir=r'D:\Test_data\Retinal_images\HRF', transform=composed)

    seg_ds = hrf_ds # ConcatDataset((hrf_ds, drive_ds))
    dataloader = DataLoader(seg_ds, batch_size=8, shuffle=True, num_workers=0)

    for i_batch, (image, target) in enumerate(dataloader):
        print(i_batch, image.shape, target[0])
        fig, ax = plt.subplots()
        ax.imshow(image[0].permute(1, 2, 0))
        p_x, p_y, v_x, v_y, d_x, d_y, a = target[0]
        circle = Circle((p_x, p_y), d_x)
        ax.add_patch(circle)
        plt.show()